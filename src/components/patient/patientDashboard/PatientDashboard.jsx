...
const PatientDashboard = () => {
  ...
  const getdoctors = async () => {
    try {
      const dataset = await axios.get("https://doctor-project-backend-r5in.onrender.com/getapproveddoctors");
        ...
        doctorDetails.map((item, index) => (
          <div key={index}>
            <div className='itemclass'>
              <img className='patientdashboardimage' src={`https://doctor-project-backend-r5in.onrender.com/` + item.profile_image} alt={item.name}></img>
